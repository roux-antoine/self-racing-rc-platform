...
from self_racing_car_msgs.msg import Connection, Graph, Node, Service, Topic


class Grapher(object):
    """Publishes a latched self_racing_car_msgs/Graph message describing the system state.
    ...
    def __init__(self, name=None):
        ...
        if not re.findall("^/*(.+)$", rospy.get_name())[0] == self._NAME:
            raise rospy.ROSInitException(
                "Node '%s' of type rosprofiler/rosgrapher should only use the name '%s' to avoid being run multiple times."
                % (rospy.get_name(), self._NAME)
            )
    ...
    def _poller_callback(self, event):
        ...
        for name in allCurrentNodes:
            ...
            try:
                ...
                rospy.logerr(
                    "WARNING: MasterAPI Error trying to contact '%s', skipping" % name
                )
                continue
            else:
                nodes[name] = node
        ...
        for topic_name, publisher_list in systemstate[0]:
            ...
            for publishername in publisher_list:
                if publishername not in nodes:
                    rospy.logwarn(
                        "Node '%s' was not previously reported, "
        ...
        for service_name, provider_list in systemstate[2]:
            for providername in provider_list:
                if providername not in nodes:
                    rospy.logwarn(
                        "Node '%s' was not previously reported, but "
                ...
                try:
                    ...
                    rospy.logerr(
                        "WARNING: MasterAPI Error trying to lookup service '%s', skipping"
                        % service_name
                    )
                    continue
                else:
        ...
        for node in nodes.values():
            try:
                node_proxy = client.ServerProxy(node.uri)
                ...
                rospy.logerr(
                    "WANRING: XML RPC ERROR contacting '%s', skipping" % node.name
                )
                continue
