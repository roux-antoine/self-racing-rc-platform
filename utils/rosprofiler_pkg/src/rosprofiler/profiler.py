...
from self_racing_car_msgs.msg import HostStatistics, NodeStatistics
...
def get_sys_hostname():
    ...
    return (
        hostname
        if is_legal_name(hostname)
        else hashlib.md5(hostname.encode()).hexdigest()[:6]
    )
...
class Profiler(object):
    ...
    def __init__(self, sample_rate=None, update_rate=None):
        ...
        self._node_publisher = rospy.Publisher(
            "node_statistics", NodeStatistics, queue_size=10
        )
        self._host_publisher = rospy.Publisher(
            "host_statistics", HostStatistics, queue_size=10
        )
            ...
            for name in nodenames:
                if name not in self._nodes:
                    ...
                    try:
                        ...
                        code, msg, pid = client.ServerProxy(uri).getPid("/NODEINFO")
                        node = NodeMonitor(name, uri, pid)
                    except rosgraph.masterapi.MasterError:
                        rospy.logerr(
                            "WARNING: MasterAPI Error trying to contact '%s', skipping"
                            % name
                        )
                        continue
                    except client.socket.error:
                        rospy.logerr(
                            "WANRING: XML RPC ERROR contacting '%s', skipping" % name
                        )
                        continue
