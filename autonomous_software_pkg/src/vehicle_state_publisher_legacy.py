...
class VehicleStatePublisher:
    def __init__(self):
        rospy.init_node("vehicle_state_publisher_legacy", anonymous=True)

        self.sub = rospy.Subscriber("gps_info", Gprmc, self.callback, queue_size=10)
        self.pub = rospy.Publisher("vehicle_state_legacy", VehicleState, queue_size=10)
    ...
    def callback(self, rmc_msg):
        ...
        angle = -rmc_msg.track * math.pi / 180
