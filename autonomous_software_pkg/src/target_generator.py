...
class TargetGenerator:
    def __init__(self):
        ...
        """ Subscribers """
        rospy.Subscriber("waypoints", WaypointArray, self.callback_waypoints, queue_size=10)
        rospy.Subscriber("current_pose", PoseStamped, self.callback_current_pose, queue_size=10)
        rospy.Subscriber("current_velocity", TwistStamped, self.callback_current_velocity, queue_size=10)
        rospy.Subscriber("manual_input_speed", TwistStamped, self.callback_manual_input_speed, queue_size=10)

        """ Publishers """
        self.target_point_pub = rospy.Publisher("target_point", PoseStamped, queue_size=10)
        self.target_velocity_pub = rospy.Publisher("target_velocity", TwistStamped, queue_size=10)
        # self.target_gen_debug_pub = rospy.Publisher("target_generator_debug", TBD, queue_size=10)
        self.target_point_marker_pub = rospy.Publisher("target_point_marker", Marker, queue_size=10)
    ...
    def callback_current_pose(self, pose_msg: PoseStamped):
        ...
        if self.waypoints == None:
            ...
            rospy.logwarn("Waypoints haven't been acquired yet. Can't process new pose.")
    ...
    def getNextWaypoint(self):
        ...
        for wp in self.waypoints.waypoints:
            distance = plane_distance(State(x = wp.pose.position.x, y = wp.pose.position.y), self.current_state)
            if distance < d_min:
                d_min = distance
                id_closest_wp = wp.id
        ...
        for wp in self.waypoints.waypoints[id_closest_wp + 1 :]:
            if plane_distance(State(x = wp.pose.position.x, y = wp.pose.position.y), self.current_state) > self.lookahead_distance:
                return wp.id
    ...
    def getTargetPoint(self, nextWaypointId: int):
        ...
        intersections = circle_line_segment_intersection(
            ...
            pt1=(self.waypoints.waypoints[nextWaypointId].pose.position.x, self.waypoints.waypoints[nextWaypointId].pose.position.y),
            pt2=(self.waypoints.waypoints[nextWaypointId - 1].pose.position.x, self.waypoints.waypoints[nextWaypointId - 1].pose.position.y)
        )

        # 2 intersections
        if len(intersections) == 2:
            # Distance between NextWaypoint and first intersection
            d1 = plane_distance(State(x = self.waypoints.waypoints[nextWaypointId].pose.position.x,
                                      y = self.waypoints.waypoints[nextWaypointId].pose.position.y),
                                State(x = intersections[0][0],
                                      y = intersections[0][1])
                                )
            # Distance between NextWaypoint and second intersection
            d2 = plane_distance(State(x = self.waypoints.waypoints[nextWaypointId].pose.position.x,
                                      y = self.waypoints.waypoints[nextWaypointId].pose.position.y),
                                State(x = intersections[1][0],
                                      y = intersections[1][1])
                                )
        ...
        # One intersection, the target point is the NextWaypoint
            ...
            target_point = State(x = self.waypoints.waypoints[nextWaypointId].pose.position.x,
                                 y = self.waypoints.waypoints[nextWaypointId].pose.position.y)
